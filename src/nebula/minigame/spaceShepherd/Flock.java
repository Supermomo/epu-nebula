...
public class Flock extends SteeringEntity {
	...
	public boolean isEnded(final ArrayList<Line> fences, Vector2f target) {
		...
		Line bottom = new Line(margin, super.getYfield() - margin, super
				.getXfield()
				- margin, super.getYfield() - margin);
		Line right = new Line(super.getXfield() - margin, margin, super
				.getXfield()
				- margin, super.getYfield() - margin);
		...
		/*
		 * ArrayList<Line> remaining=new ArrayList<Line>(); for(Line le:fences){
		 * remaining.add(le); }
		 */
			...
			if (inter != null || interBord != null) {
				...
				if (inter != null) {
					...
					for (int e = 0; e < interList.size(); e++) {
						...
						if (h != shape.size() - 1) {
							Vector2f point = interList.get(e + 1).intersect(l,
									true);
					...
					// Point in the middle of the line formed by the two bord
					// intersection
					Vector2f middle = new Vector2f(
							(interBord[0].x + interBord[1].x) / 2,
							(interBord[0].y + interBord[1].y) / 2);

					System.out.println(" middle : x " + middle.x + " y : "
							+ middle.y);
					// Closest corner to that point
					Vector2f closesstCorner = getClosestCorner(middle);

					System.out.println("closest : x : " + closesstCorner.x
							+ " y : " + closesstCorner.y);
					...
					System.out.println("indice : "
							+ remaining.indexOf(shape.get(indexFirstLine)));
					// Ajout du coin de départ et de la premiere ligne (du coin
					// a la premiere intersection avec le bord)
					path.moveTo(closesstCorner.x, closesstCorner.y);
					path.lineTo(shape.get(indexFirstLine).getX1(), shape.get(
							indexFirstLine).getY1());
					...
					// On retire les ligne de la forme crée de la list pour le
					// pas les traiter 2 fois.
	...
	private Vector2f getClosestCorner(Vector2f point) {
		...
		tab[3] = new Vector2f(super.getXfield() - margin, super.getYfield()
				- margin);
	 ...
	 * @return
	 */
	private Vector2f[] intersectBord(ArrayList<Line> shape,
			ArrayList<Line> Bords) {
